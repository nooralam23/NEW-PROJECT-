...
<html>
    ...
    <body class="skin-black">
        ...
        <div class="wrapper row-offcanvas row-offcanvas-left">
                            ...
                            <div class="box">
                                <div class="box-header">
                                    <div style="padding:10px;">
                                        ...
                                        <button class="btn btn-primary btn-sm" data-toggle="modal" data-target="#addFacultyModal"><i class="fa fa-plus" aria-hidden="true"></i> Add Faculty</button>  
                                ...
                                <form method="post">
                                    <table id="faculty_table" class="table table-bordered table-striped">
                                        ...
                                        <tbody>
                                            <?php
                                            $squery = mysqli_query($con, "select *,c.id as cid from tblfaculty f left join tblcourse c on c.id = f.courseid");
                                            while($row = mysqli_fetch_array($squery))
                                            {
                                                echo '
                                                <tr>
                                                    <td><input type="checkbox" name="chk_delete[]" class="chk_delete" value="'.$row['facNo'].'" /></td>
                                                    <td>'.$row['facNo'].'</td>
                                                    <td>'.$row['lname'].', '.$row['fname'].' '.$row['mname'].'</td>
                                                    <td>'.$row['courseName'].' - '.$row['description'].'</td>
                                                    <td>'.$row['username'].'</td>
                                                    <td><button class="btn btn-primary btn-sm" data-target="#editFacultyModal'.$row['facNo'].'" data-toggle="modal"><i class="fa fa-pencil-square-o" aria-hidden="true"></i> Edit</button></td>
                                                </tr>
                                                ';

                                                include "editfaculty_modal.php";
                                            }
                                            ?>
                                        </tbody>
                                    </table>
            ...
            <?php include "addfaculty_modal.php"; ?>

            <?php include "manage_faculty_function.php"; ?>
